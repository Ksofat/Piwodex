...
export default function DyskusjeScreen({ route }) {
    ...
    const [visitedShops, setVisitedShops] = useState(new Set());
    ...
    const fetchVisitedShops = async () => {
        try {
            const response = await fetch(`https://pajace.azurewebsites.net/api/visited/user/${user.user_id}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
            const visitedStoreIds = new Set(data.map(item => item.store_id));
            setVisitedShops(visitedStoreIds);
        } catch (error) {
            console.error('Failed to fetch visited shops:', error);
        }
    };

    const fetchShopsWithDiscussions = async () => {
        try {
            const response = await fetch('https://pajace.azurewebsites.net/api/discussions-earliest-by-store');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
    ...
    const fetchUserName = async (userId) => {
        try {
            const response = await fetch(`https://pajace.azurewebsites.net/api/users/${userId}`);
    ...
    const fetchDiscussionsForStore = async (store_id) => {
        try {
            const response = await fetch(`https://pajace.azurewebsites.net/api/discussions/store/${store_id}`);
            if (!response.ok) {
    ...
    const submitComment = async () => {
        ...
        try {
            const response = await fetch('https://pajace.azurewebsites.net/api/discussions', {
                method: 'POST',
                headers: {
    ...
    const logVisit = async (storeId, userId, storeName) => {
        try {
            const response = await fetch('https://pajace.azurewebsites.net/api/visited', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        <ScrollView style={[styles.Dyskusje_root]}>
            ...
            {shops.map((shop) => (
                <TouchableOpacity key={shop.store_id} onPress={() => toggleShowDiscussions(shop.store_id)} style={styles.ShopBox}>
                    {visitedShops.has(shop.store_id) && (
                        <Image
                            source={Checkmark}
                    ...
                    {selectedStoreId === shop.store_id && (
                        <View>
                            ...
                            <TouchableOpacity
                                onPress={() => visitedShops.has(shop.store_id) && setModalVisible(true)}
                                style={visitedShops.has(shop.store_id) ? styles.addCommentButtonVisited : styles.addCommentButtonNotVisited}
                                disabled={!visitedShops.has(shop.store_id)}
                            >
...
addCommentButtonNotVisited: {
    backgroundColor: 'darkgreen', // Darker green for not visited shops
    padding: 10,
    margin: 10,
    borderRadius: 5,
    alignItems: 'center',
